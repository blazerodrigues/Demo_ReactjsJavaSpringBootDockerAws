...
<img width="960" alt="image" src="https://user-images.githubusercontent.com/96373227/187273314-5b6ce215-e865-48f9-b6a0-ce8f5957e12e.png"> 
<br/>

# Deployment

1. Spring Boot Docker Image was built using "mvn spring-boot:build-image".
2. React.js Docker Image was created by making a Dockerfile, then using VS Code's Docker Extension to Build Image.
3. Private repositories were created on AWS ECR service, to store the Docker Images on the cloud.
4. IAM user was created with an attached policy 'AmazonEC2ContainerRegistryFullAccess', to allow connection to ECR from local machine, using AWS CLI.
5. Docker Images were PUSHED from the local machine to ECR.
6. EC2 instance was created, instance type used was t2.large. Changes were made to the security group, to allow requests from local machine.
7. Docker was installed on the EC2 instance.
8. Docker Images were PULLED from ECR to the EC2 instance.
...
Local machine to ECR connection, using AWS CLI: <br/>
